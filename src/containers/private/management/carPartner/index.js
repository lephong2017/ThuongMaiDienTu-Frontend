...
import {reqLoadDataPaging, reqSearchCar, 
    reqCountData, reqFindCar, 
    reqDeleteCar, reqAddCar, reqUpdateCar} from 'redux/carPartner/actions';
...
class CarManagement extends Component{
    ...
    disableADD = () =>{
        ...
        var listRole = [];
        if(role === 'PARTNER') {
            listRole= Role_User.findRolePartner('CAR_PARTNER');
        }
        return listRole.includes("ADD");
    }
    ...
    disableDelele = () =>{
        ...
        return listRole.includes("DELETE");
    }
...
const mapStateToProps = state => {
    return {
        car: state.carPartner,
        itemCar: state.itemCarPartner,
        numberCar: state.numberCarPartner
    }
}
